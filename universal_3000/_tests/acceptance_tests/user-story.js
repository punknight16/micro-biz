...
var CREDS = require('../../../_config/creds').user_creds;
...
describe('user login', ()=>{
	it('checks the login functionality from the browser', async ()=>{
		...
		await page.screenshot({ path: 'screenshots/user-login-submit.png' });
		var token_obj = JSON.parse(token_str);
		assert(token_obj.cred_id == 'c2');
		assert(token_obj.token_id == 't2');
		assert(token_obj.public_token == 'k2');
		...
		await page.screenshot({ path: 'screenshots/user-access-submit.png' });
		...
		await page.screenshot({ path: 'screenshots/user-add-access-submit.png' });
		assert(permission_id=='"permission not found"');
